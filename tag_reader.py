...
class MainWindow(QMainWindow):
    ...
    def _show_help(self):
        QMessageBox.about(
            self, "Usage",
            "QUICK START\n"
            "Enter the PLC IP and tag name, then click Read Tag (or press Enter in any field). "
            "On success, use Open Folder on the dialog to jump straight to the saved CSV.\n\n"
            "FIELDS\n"
            " - PLC IP: IP address of the controller.\n"
            " - Tag: Full tag path to read (see Tag Format below).\n"
            " - Output File Name: Optional. Defaults to the tag name if left blank.\n"
            " - CSV Save Location: Optional. Defaults to the folder containing this tool "
            "if left blank. The folder must exist and be writable.\n\n"
            "TAG FORMAT\n"
            "Array tags use: tag_name[start]{length}\n"
            " - [start] is optional and defaults to [0].\n"
            " - {length} is optional. If omitted, only a single element is read.\n"
            "Program-scoped tags must start with Program:program_name.tag_path\n\n"
            "Examples:\n"
            " - MyArray[4]{50} — 50 elements starting at index 4\n"
            " - Program:MyProgram.MyArray[0]{10}\n"
            " - MyStruct.FieldName — single struct member or scalar\n\n"
            "The {length} syntax applies to the array portion of the tag, not mid-path. "
            "For example, MyArray[0]{7}.Status is valid; MyArray{7}.Status is not.\n\n"
            "OUTPUT FORMAT\n"
            "Array and struct data is formatted into a CSV automatically.\n"
            " - Default (wide): One row per array element, one column per field. "
            "Nested arrays (e.g. PressA_Force[0]…[10]) each get their own column.\n"
            " - Compact Wide (Options menu): Same row layout, but nested arrays are "
            "collapsed into a single comma-separated column per field. Recommended for "
            "large UDTs with many nested arrays.\n\n"
            "OPTIONS MENU\n"
            " - Output Raw File: Also saves the unformatted tag/value list as a separate "
            "_raw CSV.\n"
            " - Compact Wide Output: Collapses nested array fields into one column each.\n"
            " - Enable Debug Logging: Writes detailed connection and read info to "
            "tag_reader.log next to the tool. Use only when troubleshooting.\n\n"
            "FILE NAMING\n"
            "Existing files are never overwritten. If my_tag.csv already exists, the "
            "next save becomes my_tag_1.csv, then my_tag_2.csv, and so on. "
            "Raw files follow the same pattern with a _raw suffix.\n\n"
            "Your last-used IP, tag, file name, save path, and option settings are "
